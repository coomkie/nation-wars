...
## Where to put unit SFX (attack / die)
...
battle-overlay/public/audio/sfx/units/
  ...
  archer_attack.mp3
  archer_die.mp3
  cavalry_attack.mp3      # spriteKey is "cavalry" (folder sprites/calvary)
  cavalry_die.mp3
  mage_attack.mp3
  mage_die.mp3
  bull_attack.mp3
...
1. Attack → `sfx/unit_attack_melee.mp3` or `sfx/unit_attack_ranged.mp3` → then `sfx/unit_attack.mp3`
2. Die → `sfx/unit_die.mp3`
...
battle-overlay/public/audio/
  ...
  sfx/
    ...
    base_destroy.mp3
    units/                 ← per-unit attack/die (put your files HERE)
      {spriteKey}_attack.mp3
      {spriteKey}_die.mp3
```

Vite serves this as `/audio/...` (e.g. `/audio/sfx/units/knight_attack.mp3`).
